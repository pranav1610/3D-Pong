...
export class FinalProject extends Scene {
        ...
        this.text_image = new Material(textured,
            {ambient: 1, diffusivity: 0, specularity: 0, texture: new Texture("assets/text.png")});
                ...
                this.floor = new Material(new Shadow_Textured_Phong_Shader(1), {
                    color: hex_color("#0b518a"), ambient: .2, diffusivity: 0.6, specularity: 0.4, smoothness: 64,
                    color_texture: new Texture("assets/tt.jpg"),
                    light_depth_texture: null
                })
                // For the first pass
    ...
    display(context, program_state) {
        ...
        if(start){
            ...
            // vec3(0, 4.5, 15), vec3(0, .5, 0), vec3(0, 1, 0)
            //vec3(-4, 2, -3), vec3(-2, 1.5, -7), vec3(0, 1, 0)
            this.initial_camera_location = Mat4.look_at(vlerp(vec3(-4, 2, -3), vec3(0, 4.5, 15), Math.min(.5*t2,1)), vlerp(vec3(-2, 1.5, -7), vec3(0, .5, 0), Math.min(.5*t2,1)), vec3(0, 1, 0))
